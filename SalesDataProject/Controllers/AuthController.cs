                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        for (int row = 3; row <= lastRow; row++) // Start reading data from row 3
                        {
                            ...
                            if (!new[] { "Corporate", "CORPORATE", "LAWFIRM", "LAW FIRM", "UNIVERSITY", "University", "PCT", "Individual", "INDIVIDUAL" }.Contains(category?.ToUpperInvariant()))
                            {
                                invalidRecords.Add(new InvalidCustomerRecord
                                {
                                    RowNumber = row - 1,
        ...
        public IActionResult DownloadTemplate()
        {
            try
            {
                using (var workbook = new XLWorkbook())
                {
                    ...
                    worksheet.Cell(2, 12).Value = "Corporate/Law Firm/University/PCT/Individual";
